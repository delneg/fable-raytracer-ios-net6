...
module Native =
    ...
    extern int32 add_values(nint value1, nint value2)
    
    [<DllImport(DllName, CallingConvention=CallingConvention.Cdecl)>]
    //(x: i32, y: i32, w: i32, h: i32, angle: f64) -> *const u8
    extern IntPtr render_scene(nint x, nint y, nint w, nint h, float angle)
...
//            printfn $"Creating image for {degrees} degrees"
            ...
            let ret_ptr_A = Native.render_scene(nint xa,nint ya,nint wa,nint ha,angleA)
    ...
    override this.ViewDidLoad() =
        ...
        let angle = 0.0
        let len = w * h * 4        
        let ret_ptr = Native.render_scene(nint x,nint y,nint w,nint h,angle)
...
type AppDelegate() =
    ...
    override this.FinishedLaunching(_, _) =
        printfn $" Native add_values: 5 + 6 = {Native.add_values(nint 5,nint 6)}"
